...
This repository contains the code and data for "Multi-Turn Secret Extraction from Persona-Embedded LLM Agents" (Kharchevnikov & Flowerday), the multi-turn extension of the authors' earlier single-turn study, "Securing LLM-Powered NPCs Without Sacrificing Generative Utility" (2026). The single-turn study tested persona-embedded defenses against adversarial attacks in one exchange. This extension runs the same target NPC configuration (Optimal) against four attack families across seven-turn conversations, producing 1,000 conversations and 7,000 NPC responses.
